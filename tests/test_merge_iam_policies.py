...
SCRIPT_PATH = REPO_ROOT / "scripts" / "merge_iam_policies.py"


def ensure_python_available():
    try:
        subprocess.run([sys.executable, "--version"], check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError as exc:  # pragma: no cover - Python should always exist
        raise unittest.SkipTest("Python interpreter not available") from exc
        ...
        if expect_success:
            if result.returncode != 0:
                raise AssertionError(
                    f"Merge command failed with code {result.returncode}:\nSTDOUT:{result.stdout}\nSTDERR:{result.stderr}"
                )
        else:
            if result.returncode == 0:
...
class MergeIamPoliciesTest(unittest.TestCase):
    ...
    def test_large_output_warns(self):
        ...
        self.assertIn("Merged policy size", combined_output)
